...
st.caption("v5.2 | 20.02.2026 | Синхр. со сканером v10.2 | Adaptive Stop + MTF + CSV с датой")
...
with tab1:
    if not open_positions:
        ...
        # v5.1: CSV export for open positions
        open_rows = []
        for pos in open_positions:
            open_rows.append({
                '#': pos['id'],
                ...
                'Dir': pos['direction'],
                'TF': pos['timeframe'],
                'Entry Z': f"{pos['entry_z']:+.2f}",
                'HR': pos.get('hedge_ratio', 0),
                'Stop Z': pos.get('stop_z', 4.0),
                'Вход': pos['entry_time'][:16],
            })
        if open_rows:
            csv_open = pd.DataFrame(open_rows).to_csv(index=False)
            st.download_button("📥 Открытые позиции (CSV)", csv_open,
